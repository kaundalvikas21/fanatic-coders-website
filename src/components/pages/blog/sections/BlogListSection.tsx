    ...
    <section id="blog-list" className="py-20 md:py-24 relative overflow-hidden" style={{ background: "var(--dark-2)" }}>
      ...
      <div className="relative z-10 container mx-auto px-4 max-w-6xl">
        <RevealSection className="mb-12">
          <SectionHeading
            badge="posts.latest"
            title={<>latest.<span className="function">posts</span>()</>}
            ...
            <Link href={`/blog/${featured.slug}`} className="no-underline group/feat block">
              <GlassCard accent="violet" lift className="grid md:grid-cols-2 overflow-hidden">
                ...
                <div className="p-7 md:p-9 flex flex-col justify-center">
                  ...
                  <h2 className="mt-4 text-xl md:text-2xl font-bold text-white leading-snug">{featured.title}</h2>
                  <p className="mt-3 text-sm text-blue-100/65 leading-relaxed">{featured.excerpt}</p>
                  <div className="mt-5 flex items-center gap-4 text-xs font-mono text-blue-100/50">
                    <span>{featured.date}</span>
                    <span className="flex items-center gap-1.5"><Clock size={12} aria-hidden />{featured.readTime}</span>
                  </div>
                  <span className="mt-5 inline-flex items-center gap-1.5 text-sm font-mono text-indigo-300">
                    readPost <ArrowRight size={14} className="transition-transform group-hover/feat:translate-x-1" aria-hidden />
                  </span>
                </div>
              </GlassCard>
            </Link>
            ...
            <Link key={post.id} href={`/blog/${post.slug}`} className="no-underline group/card">
              <GlassCard lift className="h-full overflow-hidden flex flex-col">
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <p className="mt-2 text-sm text-blue-100/60 leading-relaxed flex-1">{post.excerpt}</p>
                  <div className="mt-4 flex items-center gap-3 text-xs font-mono text-blue-100/50">
                    <span>{post.date}</span>
                    <span className="flex items-center gap-1.5"><Clock size={12} aria-hidden />{post.readTime}</span>
                  </div>
                </div>
              </GlassCard>
            </Link>
        ...
        {grid.length === 0 && (
          <p className="mt-10 text-center text-sm font-mono text-blue-100/50">{"// no posts match your search yet"}</p>
        )}
      </div>
    </section>
  )
