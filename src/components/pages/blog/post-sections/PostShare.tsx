...
function BrandIcon({ path, color }: { path: string; color?: string }) {
  return (
    <svg viewBox="0 0 24 24" width={16} height={16} fill="currentColor" style={color ? { color } : undefined} aria-hidden focusable="false">
      <path d={path} />
    </svg>
  )
}
...
export function PostShare({ title }: { title: string }) {
  ...
  return (
    <div>
      <span className="mb-2 block text-xs font-mono uppercase tracking-[0.12em] text-blue-100/45">share</span>
      <div className="flex flex-wrap gap-2">
        <button type="button" onClick={copy} aria-label={copied ? "Link copied" : "Copy link to this post"} className={ICON_BTN}>
          {copied ? <Check size={16} aria-hidden /> : <Link2 size={16} aria-hidden />}
        </button>
        {canShare && (
          <button type="button" onClick={share} aria-label="Share this post" className={ICON_BTN}>
            <Share2 size={16} aria-hidden />
          </button>
        )}
        <button type="button" onClick={() => openIntent("x")} aria-label="Share on X" className={ICON_BTN}>
          <BrandIcon path={BRAND_PATH.x} color={BRAND_COLOR.x} />
        </button>
        <button type="button" onClick={() => openIntent("linkedin")} aria-label="Share on LinkedIn" className={ICON_BTN}>
          <BrandIcon path={BRAND_PATH.linkedin} color={BRAND_COLOR.linkedin} />
        </button>
        <button type="button" onClick={() => openIntent("reddit")} aria-label="Share on Reddit" className={ICON_BTN}>
          <BrandIcon path={BRAND_PATH.reddit} color={BRAND_COLOR.reddit} />
        </button>
      </div>
    </div>
  )
