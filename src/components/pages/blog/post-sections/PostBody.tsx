        ...
        switch (block.type) {
          ...
          case 'list':
            return block.ordered ? (
              <ol
                key={key}
                className="list-decimal space-y-2 pl-5 marker:text-indigo-300"
              >
                {block.items.map((it) => (
                  <li key={it}>{it}</li>
                ))}
              ...
              <ul
                key={key}
                className="list-disc space-y-2 pl-5 marker:text-indigo-300"
              >
                {block.items.map((it) => (
                  <li key={it}>{it}</li>
                ))}
          ...
          case 'quote':
            return (
              <blockquote
                key={key}
                className="rounded-lg bg-white/[0.03] px-5 py-4 text-blue-100/85 italic"
              >
                <p>{block.text}</p>
                {block.cite && (
                  <footer className="mt-2 text-xs font-mono not-italic text-blue-100/60">
                    {block.cite}
                  </footer>
                )}
              </blockquote>
          ...
          <article className="max-w-[68ch] min-w-0">
            {post.takeaways && post.takeaways.length > 0 && (
              <GlassCard
                ...
                className="p-6 mb-12"
              >
                <div className="text-xs font-mono uppercase tracking-[0.18em] text-blue-100/70 mb-3">
                  Key takeaways
                </div>
            ...
            <div>
              ...
              {sections.map((s) => (
                <div
                  key={s.id}
                  id={s.id}
                  className="scroll-mt-[var(--header-offset)] mt-12 border-t border-white/10 pt-12 first:mt-0 first:border-t-0 first:pt-0"
                >
