...
export function PostAuthor({ post }: { post: BlogPost }) {
  ...
  return (
    <section className="relative overflow-hidden py-12" style={{ background: "var(--dark-2)" }}>
      <div className="relative z-10 container mx-auto px-4 sm:px-6 max-w-3xl">
        <RevealSection>
          <GlassCard accent="cyan" className="flex flex-col sm:flex-row items-start gap-5 p-6">
            {post.authorAvatar && (
              <span className="relative h-16 w-16 flex-shrink-0 overflow-hidden rounded-2xl ring-1 ring-white/15">
                <Image src={post.authorAvatar} alt={post.author} fill className="object-cover" sizes="64px" />
              </span>
            )}
            <div>
              <div className="text-xs font-mono text-blue-100/45">written by</div>
