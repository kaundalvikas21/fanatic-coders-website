    ...
    >
      <div className="relative z-10 container mx-auto px-4 sm:px-6 max-w-5xl">
        <RevealSection className="max-w-[68ch]">
          ...
          <GlassCard
            accent="cyan"
            className="flex flex-col sm:flex-row items-start gap-5 p-6"
          >
            {post.authorAvatar && (
            ...
            <div>
              ...
              {post.authorRole && (
                <div className="text-sm font-mono text-indigo-300">{post.authorRole}</div>
              )}
              <p className="mt-3 text-sm text-blue-100/65 leading-relaxed">
                Part of the senior team at fanaticCoders, writing from the work we ship for clients
                in {post.category.toLowerCase()} and beyond.
              </p>
            </div>
          </GlassCard>
        </RevealSection>
