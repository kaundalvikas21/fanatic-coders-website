...
export function PostRelated({ post }: { post: BlogPost }) {
  ...
  return (
    <section
      className="cv-auto relative overflow-hidden section-y"
        ...
        >
          {related.map((p) => (
            <Link
              ...
              className="no-underline group/card rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/60"
            >
              <GlassCard
                lift
                ...
                {p.coverUrl && (
                  <div className="relative aspect-[16/10] overflow-hidden">
                    <Image
                      ...
                      alt={p.title}
                      fill
                      className="object-cover brightness-95 transition-[filter] duration-500 group-hover/card:brightness-110"
                      sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw"
                    />
                  </div>
                )}
                <div className="p-5 flex flex-1 flex-col">
                  <div className={`text-[11px] font-mono ${categoryColor(p.category).label}`}>
                    {p.category}
                  </div>
                  <h3 className="mt-2 text-base font-bold text-white leading-snug text-balance transition-colors group-hover/card:text-indigo-200">
                    {p.title}
                  </h3>
