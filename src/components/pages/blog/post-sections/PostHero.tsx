        ...
        <RevealSection stagger>
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-left text-sm">
                  <span className="block font-semibold text-white leading-tight">
                    <span className="sr-only">Written by </span>{post.author}
                  </span>
                  {post.authorRole && <span className="mt-0.5 block text-xs text-blue-100/65">{post.authorRole}</span>}
                </span>
              </div>
            )}
            ...
            <div className="flex items-center gap-4 text-sm font-mono tabular-nums text-blue-100/70">
              <time dateTime={isoDate}>
                <span className="sr-only">Published </span>{post.date}
              </time>
              <span className="flex items-center gap-1.5" aria-label={`Reading time ${post.readTime}`}>
                <Clock size={13} aria-hidden />{post.readTime} read
              </span>
            </div>
          </div>
          ...
          {post.tags && post.tags.length > 0 && (
            <div className="mt-6 flex flex-wrap justify-center gap-2">
              {post.tags.map((tag) => (
                <Link key={tag} href={`/blog?tag=${encodeURIComponent(tag)}`} aria-label={`View posts tagged ${tag}`} className="inline-flex items-center min-h-[44px] rounded-full bg-white/10 px-3 py-2 text-xs font-mono tracking-[0.04em] text-blue-100/85 ring-1 ring-white/15 backdrop-blur-sm transition-colors hover:bg-indigo-500/20 hover:text-white hover:ring-indigo-400/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/60 focus-visible:text-white">#{tag}</Link>
              ))}
            </div>
          )}
        </RevealSection>
