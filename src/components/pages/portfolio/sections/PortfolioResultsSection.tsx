      ...
      <div className="relative z-10 container mx-auto px-4">
        <RevealSection>
          <SectionHeading
            ...
            title={
              <>
                the.<span className="function">numbers</span>()
              </>
            }
          ...
          <div className="grid grid-cols-2 gap-y-10 text-center sm:flex sm:items-center sm:justify-between sm:gap-0 sm:divide-x sm:divide-white/10">
            {stats.map((s) => (
              ...
              >
                <CountUp
                  value={s.value}
                  className={`block font-bold font-mono tabular-nums leading-none ${s.featured ? 'text-4xl md:text-5xl text-aurora-sweep' : 'text-3xl md:text-4xl text-white'}`}
                />
