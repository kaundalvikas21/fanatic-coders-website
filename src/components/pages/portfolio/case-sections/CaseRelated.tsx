                    ...
                    <div className="relative aspect-[16/10] overflow-hidden border-b border-white/15">
                      ...
                      {top && (
                        <div className="absolute left-3 top-3 rounded-xl bg-black/45 px-3 py-2 ring-1 ring-white/10 backdrop-blur">
                          <div className={`text-sm font-bold font-mono tabular-nums ${a.text}`}>
                            {top.value}
                          </div>
                          {top.caption && (
                            <div className="text-[10px] text-white/60">{top.caption}</div>
                          )}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="p-5">
                    <div className="flex flex-wrap gap-1.5">
                      {p.tags.slice(0, 2).map((tag) => (
                        <span
                          key={tag}
                          className="rounded-full bg-white/[0.06] px-2.5 py-0.5 text-[11px] font-mono text-blue-100/70"
                        >
                          {tag}
                        </span>
                      ))}
                    ...
                    <span
                      className={`mt-4 inline-flex items-center gap-1.5 text-sm font-mono opacity-70 transition-opacity group-hover/c:opacity-100 ${a.text}`}
                    >
                      view case study
                      <ArrowUpRight
                        size={15}
