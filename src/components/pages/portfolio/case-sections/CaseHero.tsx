...
function MetaRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-4 py-3.5">
      <dt className="text-[11px] font-mono uppercase tracking-[0.14em] text-blue-100/40">{label}</dt>
...
export function CaseHero({ project }: { project: PortfolioProject }) {
  ...
  return (
    <section className="relative flex min-h-[100svh] flex-col overflow-hidden pt-[184px] md:pt-[196px] pb-16">
      {/* Background */}
      {hasImage ? (
        <>
          <Image src={project.imageUrl!} alt={`${project.title} cover`} fill priority sizes="100vw" className="absolute inset-0 -z-20 object-cover" />
          ...
          {hasMeta && (
            <RevealSection>
              <div className="rounded-2xl border border-white/12 bg-white/[0.05] p-6 md:p-7 backdrop-blur-xl shadow-[0_8px_40px_rgba(0,0,0,0.35),inset_0_1px_0_rgba(255,255,255,0.07)]">
                <div className="border-b border-white/10 pb-4 text-[11px] font-mono uppercase tracking-[0.18em] text-blue-100/40">{"// at a glance"}</div>
                <dl className="divide-y divide-white/[0.07] pt-1">
                  {project.client && <MetaRow label="client" value={project.client} />}
                  {project.industry && <MetaRow label="industry" value={project.industry} />}
                  {project.year && <MetaRow label="year" value={project.year} />}
                  {project.duration && <MetaRow label="duration" value={project.duration} />}
                </dl>
                {project.services && project.services.length > 0 && (
                  <div className="mt-1 border-t border-white/10 pt-4">
                    <div className="text-[11px] font-mono uppercase tracking-[0.14em] text-blue-100/40">services</div>
                    <div className="mt-2.5 flex flex-wrap gap-1.5">
                      {project.services.map((s) => (
                        <span key={s} className="rounded-md border border-white/10 bg-white/[0.06] px-2.5 py-1 text-xs text-blue-100/85">{s}</span>
                      ))}
                    </div>
                  </div>
                )}
