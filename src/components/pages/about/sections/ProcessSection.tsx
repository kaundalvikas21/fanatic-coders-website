...
const steps: Step[] = [
  {
    ...
    title: 'Discover',
    description:
      'We dig into goals, users, and constraints, then turn them into a sharp, prioritized scope.',
    Icon: Search,
  },
  {
    n: '02',
    title: 'Design',
    description:
      'Flows, wireframes, and a polished UI in your design system, validated before we build.',
    Icon: PenTool,
  },
  {
    n: '03',
    title: 'Build',
    description:
      'Senior engineers ship in weekly increments with demos, tests, and CI from day one.',
    Icon: Code2,
  },
  {
    n: '04',
    title: 'Launch & iterate',
    description: 'We deploy, measure, and refine, and stay on as partners well past go-live.',
    Icon: Rocket,
  },
];
      ...
      <div className="relative z-10 container mx-auto px-4">
        <RevealSection>
          <SectionHeading
            ...
            title={
              <>
                how.<span className="function">weShip</span>()
              </>
            }
            ...
            {steps.map((step) => (
              ...
              >
                <span className="relative z-10 inline-flex h-12 w-12 items-center justify-center rounded-full bg-[var(--dark-2)] ring-1 ring-white/10 transition-shadow duration-300 group-hover:ring-white/20 group-hover:shadow-[0_0_22px_-4px_var(--aurora-cyan-light)]">
                  <span className="process-num text-3xl font-bold font-mono tabular-nums">
                    {step.n}
                  </span>
                </span>
                <div className="mt-5 flex items-center justify-center gap-2">
                  <step.Icon
                    size={18}
                    className="shrink-0 text-[var(--aurora-cyan-light)]"
                    aria-hidden
                  />
                  <h3 className="text-lg font-bold text-[var(--color-text-base)]">{step.title}</h3>
                </div>
                <p className="mt-2 max-w-[28ch] text-sm text-[var(--color-text-muted)] leading-relaxed">
                  {step.description}
                </p>
              </li>
            ))}
