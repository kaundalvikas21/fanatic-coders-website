...
const cards: CardConfig[] = [
  { slug: "web-development", accent: "violet", bullets: ["Server-rendered React", "Type-safe APIs", "Performance budgets", "Accessible by default"] },
  { slug: "ecommerce", accent: "cyan", bullets: ["Headless commerce", "Payments and tax", "Cart and checkout", "Inventory sync"] },
  { slug: "mobile-apps", accent: "violet", bullets: ["React Native", "Offline support", "Push notifications", "Store release"] },
  { slug: "design", accent: "cyan", bullets: ["Design systems", "Prototypes", "User research", "Handoff specs"] },
  { slug: "cloud", accent: "green", bullets: ["CI/CD pipelines", "Containers", "Monitoring", "Cost control"] },
]
    ...
    <section id="services-overview" className="section-y relative overflow-hidden" style={{ background: "var(--dark-2)" }}>
      ...
      <div className="relative z-10 container mx-auto px-4">
        <RevealSection>
          <SectionHeading
            badge="what.we.do"
            title={<>our.<span className="function">services</span>()</>}
            comment="// five core practices, one team"
          />
        </RevealSection>

        <RevealSection stagger className="mt-16 grid sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-6xl mx-auto">
          {cards.map((card) => {
            ...
            return (
              <Link key={card.slug} href={`/services/${card.slug}`} className="no-underline group/card block">
                <div className="bento-card rounded-2xl p-6 h-full" style={accentVars(card.accent)}>
                  <div className="relative z-[1] flex flex-col h-full">
                    <div className="icon-box mb-4 flex h-11 w-11 items-center justify-center rounded-xl">
                      <service.Icon size={20} style={{ color }} aria-hidden />
                    </div>
                    <h3 className="text-base font-bold text-white mb-2">{service.title}</h3>
                    <p className="text-sm text-blue-100/60 leading-relaxed mb-4">{service.description}</p>
                    <ul className="space-y-2 mb-6">
                      {card.bullets.map((bullet) => (
                        <li key={bullet} className="flex items-center gap-2 text-sm text-blue-100/70">
          ...
          <div className="bento-card rounded-2xl p-6 h-full" style={accentVars("violet")}>
