...
export function ServiceProcess() {
  ...
  return (
    <section className="relative overflow-hidden section-y cv-auto" style={{ background: "var(--dark-1)" }}>
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 max-w-6xl">
        ...
        <RevealSection as="ol" stagger className="mt-14 grid list-none sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {serviceProcess.map((step) => {
            const ongoing = step.duration === "ongoing"
            return (
              <li key={step.n} className="h-full">
                <GlassCard lift className={cn("process-card group flex h-full flex-col p-7", ongoing && "process-card-ongoing")}>
