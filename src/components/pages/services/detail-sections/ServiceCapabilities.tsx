...
export function ServiceCapabilities({ service, group }: { service: ServiceItem; group: ServiceGroup }) {
  ...
  return (
    <section className="relative overflow-hidden section-y cv-auto" style={{ background: "var(--dark-2)" }}>
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 max-w-6xl">
        ...
        <RevealSection as="ul" stagger className="mt-14 grid list-none sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {capabilities.map((cap) => (
            <li key={cap.title} className="bento-card cap-card rounded-2xl p-6 h-full" style={accentVars(group.accent)}>
          ...
          <li className="bento-card cap-card rounded-2xl p-6 h-full" style={accentVars(group.accent)}>
