...
export function ServiceHero({ service, group }: { service: ServiceItem; group: ServiceGroup }) {
  ...
  const stats = service.stats.length > 0 && (
    <RevealSection
      stagger
      className="mt-12 flex flex-wrap gap-x-8 gap-y-5 max-w-3xl"
    >
      {service.stats.map((s) => (
        <div
          key={s.label}
          className="flex flex-col sm:[&:not(:first-child)]:border-l sm:[&:not(:first-child)]:border-white/10 sm:[&:not(:first-child)]:pl-8"
        >
          <span className="text-3xl md:text-4xl font-bold font-mono tabular-nums text-white">
            {s.value}
          </span>
          <span className="mt-1 text-xs font-mono uppercase tracking-wider text-blue-100/55">
            {s.label}
          </span>
        </div>
      ))}
