  ...
  return (
    <section className="relative overflow-hidden section-y" style={{ background: "var(--dark-2)" }}>
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 max-w-6xl">
        ...
        <RevealSection stagger className="mt-10 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {related.map((item) => (
            <Link key={item.slug} href={`/services/${item.slug}`} className="no-underline group/card">
              <GlassCard accent={group.accent} lift className="h-full p-6">
                <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
                  <item.Icon size={20} style={{ color: accent }} aria-hidden />
                </div>
                <h3 className="text-base font-bold text-white mb-2">{item.title}</h3>
                <p className="text-sm text-blue-100/60 leading-relaxed mb-4">{item.description}</p>
                <span className="inline-flex items-center gap-1.5 text-sm font-mono" style={{ color: accent }}>
                  explore
